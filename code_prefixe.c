...
void afficher(uint32_t *tableau, int n) 
{ 
	for (int i = 0; i < n; ++i) {
		printf("%d", tableau[i]);
        }
...
void afficher_code(noeud* racine, uint32_t *tableau, uint32_t indice) { 
 ...
 if(indice > hauteur_max_arbre){
	...
	if (racine->gauche != NULL) { 
		tableau[indice] = 0; 
		afficher_code(racine->gauche, tableau, indice + 1); 
	}  
	if (racine->droite != NULL) { 
		tableau[indice] = 1; 
		afficher_code(racine->droite, tableau, indice + 1); 
	} 
...
int main(int argc, const char *argv[])
{
  ...
  printf("\n----------------------------------Code_prefixe commence-------------------------------------------------------------------------\n");
    ...
    uint32_t indice = 0; 
	afficher_code(racine, tableau, indice);  
    free(tableau);
    free(racine);
    return 0;
}
